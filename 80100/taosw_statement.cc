    ...
    if (S->stmt && param->is_param) {
        switch (event_type) {
            ...
            case PDO_PARAM_EVT_EXEC_PRE:
                ...
                b = (PDO_TAOSW_PARAM_BIND *) param->driver_data;
                *b->is_null = 0;
...
static int pdo_taosw_stmt_get_col(pdo_stmt_t *stmt, int colno, zval *result, enum pdo_param_type *type)
{
    unsigned long *len;
    char **ptr;
    pdo_taosw_stmt *S = (pdo_taosw_stmt *) stmt->driver_data;
...
const struct pdo_stmt_methods taosw_stmt_methods = {
    ...
    pdo_taosw_stmt_describe,         /* column information */
    pdo_taosw_stmt_get_col,          /* retrieves pointer and size of the value for a column */
    pdo_taosw_stmt_param_hook,       /* param hook */
    NULL,                           /* set_attr */
    NULL,                           /* get_attr */
    pdo_taosw_stmt_get_column_meta,  /* retrieves meta data for a numbered column */
    NULL,                           /* next_rowset */
    NULL                            /* cursor_closer */
};